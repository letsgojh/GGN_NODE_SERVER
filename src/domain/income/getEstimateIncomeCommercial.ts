...
//경쟁 대비 시장의 규모를 알려준다.
export default async function getSeoulEstimateIncome_commercial() : Promise<getSeoulEstimateIncome_Param[]>{
  let startIndex = 1
  let endIndex = 1000
  let cnt : number = 0
  let url = `http://openapi.seoul.go.kr:8088/${process.env.SEOUL_API_KEY}/json/VwsmTrdarSelngQq/${startIndex}/${endIndex}/20241`
  ...
  while(true){
      url = `http://openapi.seoul.go.kr:8088/${process.env.SEOUL_API_KEY}/json/VwsmTrdarSelngQq/${startIndex}/${endIndex}/20241`
