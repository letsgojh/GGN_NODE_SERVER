import { getSeoulFloatingPopulation_commercial, getSeoulFloatingPopulation_hinterland, getSeoulCompanyPopulation_commercial, getSeoulCompanyPopulation_hinterland, getResidentPopulation_commercial, getResidentPopulation_hinterland, getSeoulMarketCount_commercial, getSeoulMarketCount_hinterland, getSeoulEstimateIncome_commercial, getSeoulEstimateIncome_hinterland, } from '../../domain/calledData.js';
//하나의 상권에 대해 로직구현
...
//name : 상권 하나의 이름
export async function getTotalPopPerStore_commercial_one_sanggwon(name) {
    //상권별 유동인구
    const seoulFloatingPopulation = await getSeoulFloatingPopulation_commercial();
    //상권 하나의 유동인구 수 담을 변수(일단 총 유동인구만)
    let totalFloatingPop = 0;
    //상권별 직장인구
...
//name : 상권 하나의 이름
export async function getTotalPopPerStore_hinterland_one_sanggwon(name) {
    //상권별 유동인구
...
export async function getPredictedIncomePerPop_commercial_one_sanggwon(name) {
    //추정매출(상권)
    const predictedIncome = await getSeoulEstimateIncome_commercial();
    //당월 추정 매출 (업종 고려x)
    let totalIncome = 0;
    //한 상권 유동인구
    const seoulFloatingPopulation = await getSeoulFloatingPopulation_commercial();
...
export async function getPredictedIncomePerPop_hinterland_one_sanggwon(name) {
    //추정매출(상권)
    const predictedIncome = await getSeoulEstimateIncome_hinterland();
    //당월 추정 매출 합(업종 고려x)
    let totalIncome = 0;
    //상권별 유동인구
    const seoulFloatingPopulation = await getSeoulFloatingPopulation_hinterland();
