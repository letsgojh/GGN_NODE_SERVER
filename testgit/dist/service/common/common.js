...
export async function getTotalPopPerStore_commercial(auto, admin) {
...
export async function getTotalPopPerStore_hinterland(auto, admin) {
    //지역의 상권이름 string[] 형태로 반환
...
//추정매출(상권) / 총 직장인구(상권)
//추정매출(상권) / 총 상주인구(상권)
export async function getPredictedIncomePerPop_commercial(auto, admin) {
    //추정매출(상권)
...
export async function getPredictedIncomePerPop_hinterland(auto, admin) {
    //추정매출(상권)
    const predictedIncome = await getSeoulEstimateIncome_commercial();
    //당월 추정 매출 합(업종 고려x)
    let totalIncome = 0;
