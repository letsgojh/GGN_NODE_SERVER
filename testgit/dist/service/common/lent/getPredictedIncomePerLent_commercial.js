import { getSeoulStorePrice, getSeoulEstimateIncome_district, } from '../../../domain/calledData.js';
...
export async function getPredictedIncomePerLent_commercial(gu) {
    let lentMap = new Map();
    let incomeMap = new Map();
    const incomeMapPlusCount = new Map();
    let regionMap = new Map();
    let oneLentPerPredictedIncome = 0;
    let totalLentPerPredictedIncome = 0;
    let count = 0;
    if (!fileExists("lentPerPyeong")) {
        console.log("lentPerPyeong파일 없어서 생성합니다.");
        const lentList = await getSeoulStorePrice();
    ...
    if (!fileExists("guPredictedIncome")) {
        ...
        const incomeList = await getSeoulEstimateIncome_district();
...
async function main() {
    ...
    const res = await getPredictedIncomePerLent_commercial("종로구");
    console.log(res);
}
main();
