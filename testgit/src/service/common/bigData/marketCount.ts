...
// 핵심 함수
export async function pushSummationMarketCount_commercial(): Promise<SummationRow[]> {
  console.log("상권별 점포수 받아오는 중...");
  const raw: getSeoulMarketCount_Param[] = await getSeoulMarketCount_commercial();
...
export async function pushSummationMarketCount_hinterland(): Promise<SummationRow[]> {
  console.log("상권별 점포수 받아오는 중...");
  const raw: getSeoulMarketCount_Param[] = await getSeoulMarketCount_hinterland();
